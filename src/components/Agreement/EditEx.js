...
export default class EditEx extends Component {
  ...
  componentWillMount() {
    let mode;
    this.setState({ open: this.props.find });
  }

  handleOpen = () => {
    this.setState({ open: false })
    this.writeLog('Agree')
    api.passengerAgree(this.props.tripId, this.props.condition, this.props.user, this.state.open)
    
  }

  handleClose = () => {
    this.setState({ open: true })
    this.writeLog('Disagree')
    api.passengerAgree(this.props.tripId, this.props.condition, this.props.user, this.state.open)
  }
      ...
      <Grid columns={2}>
        <Grid.Column>
          <Portal
            ...
            trigger={(
              <Button
                content={open ? 'Click to Agree' : 'Click to Disagree'}
                negative={open}
                positive={!open}
              />
            )}
